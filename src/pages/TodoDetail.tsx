...
export default function TodoDetail() {
  ...
  async function handleAddComment(e: FormEvent) {
    e.preventDefault()
    if (!userId || !id || !newComment.trim()) return
  ...
  if (!id) {
    return (
      <div className="min-h-screen bg-slate-50 p-6">
        <p className="text-red-600">Ungültiger Karten-Link.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      ...
      {todo && (
        <div className="bg-white border rounded p-4 mb-6 max-w-md space-y-2">
          {!editing && (
            <>
              {todo.beschreibung && <p className="text-sm text-slate-700 whitespace-pre-wrap">{todo.beschreibung}</p>}
          ...
          {editing && (
            <form onSubmit={handleSaveEdit} className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setEditing(false)} className="text-sm text-slate-600 underline">
                  Abbrechen
                </button>
              </div>
            </form>
      ...
      <ul className="space-y-2 max-w-md mb-3">
        {comments.map((c) => (
      ...
      <form onSubmit={handleAddComment} className="space-y-2 bg-white border rounded p-3 max-w-md mb-6">
        <textarea
      ...
      <form onSubmit={handleUploadDocument} className="space-y-2 bg-white border rounded p-3 max-w-md">
        <input
          type="file"
