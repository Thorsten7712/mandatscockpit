...
- `list_documents(parent_id?, sichtbarkeit?, ebene?, tag?, session_id?, limit?)` – ohne `parent_id` nur Top-Level-Dokumente (z. B. Sitzungsvorlagen), mit `parent_id` die daran angehängten Notizen/Analysen. Eigene sowie sichtbare geteilte/individuell freigegebene Dokumente anderer Mitglieder – der MCP-Server läuft über den Service-Role-Client (RLS greift dort nicht), die Sichtbarkeitsprüfung passiert deshalb explizit im Tool-Code, nicht per RLS. Die Ausgabe nennt zusätzlich über `link_todo_document` verknüpfte ToDo-Karten. `archiv` steuert den Umgang mit archivierten Dokumenten (`ohne` = Standard wie in der Web-UI, `alle`, `nur`); archivierte Treffer sind in der Ausgabe mit `ARCHIV` markiert. Als archiviert gilt ein Dokument, dessen verknüpfte Sitzung vorbei ist **oder** das per `update_document_archiv` von Hand archiviert wurde.
- `search(query, limit?)` – Volltextsuche über Titel/Beschreibung von ToDos, Titel/Inhalt von Anträgen und Notiz-Texten (eigene + geteilte). Durchsucht **nicht** den Dokumenten-Hub, dafür `list_documents` mit Filtern verwenden – und ebenso wenig die „Zahlen & Fakten"-Seite (`/fakten`, Tabelle `fakten`), für die es bislang keine MCP-Tools gibt.

**Bearbeiten:**
- `complete_todo(todo_id, erledigt?)` – abhaken/rückgängig machen.
- `update_todo(todo_id, titel?, beschreibung?, faellig_am?, zustaendig?, session_id?, spalte?)` – `session_id` verknüpft/löst die Karte nachträglich mit einer Sitzung (leerer String entfernt eine bestehende Verknüpfung); `spalte` verschiebt nur die eigene Platzierung des aufrufenden Nutzers.
...
- `update_document_archiv(dokument_id, archivieren)` – legt ein eigenes Dokument von Hand ins Archiv (`archivieren=true`) oder holt es zurück (`false`). Betrifft nur die manuelle Archivierung: ein Dokument, dessen verknüpfte Sitzung vorbei ist, bleibt archiviert und lässt sich nur über das Lösen der Sitzungs-Verknüpfung (`update_document_session`) zurückholen – darauf weist die Rückmeldung ausdrücklich hin. Nur der/die Ersteller\*in darf das.
